...
# Pre-populated Json Data based on your new resume text
INITIAL_DATA = {
  "header": {
  ...
  "expertise": {
    "System Applications & Customer Engineering": "System architecture | Customer requirements | Product evaluation | Integration & bring-up | Validation | Technical support | Root-cause analysis | Design reviews | Customer training",
    "RF & Semiconductor Systems": "Radar SoCs | RF TX/RX chains | ADC/DAC | RF characterization | Gain/noise/dynamic range | Linearity & spurious analysis | Hardware/software co-debug",
    "Software & Tools": "Python | MATLAB/Simulink | Embedded C | Java | C++ (basic) | Git | Automated validation | System modelling | Test frameworks",
    "Signal Processing": "FMCW & pulsed radar | FFT | Digital filtering | Doppler | MIMO | Beamforming | CFAR | Detection | Angle estimation",
    "Laboratory": "Oscilloscopes | Signal generators | Spectrum analyzers | Power meters | Attenuators | Frequency multipliers | RF measurements"
  },
  "experience": [
    {
...
class ResumeApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Resume Editor & Page Count Meter")
    ...
    def create_widgets(self):
        ...
        # Live Page Count Button & Label
        ttk.Button(top_frame, text="Calculate Page Count", command=self.update_page_count).pack(side=tk.LEFT, padx=15)
        self.page_label = ttk.Label(top_frame, text="Total Pages: --", font=("Helvetica", 10, "bold"), foreground="navy")
        self.page_label.pack(side=tk.LEFT, padx=5)

        ttk.Button(top_frame, text="Generate PDF", command=self.generate_pdf).pack(side=tk.RIGHT, padx=5)
    ...
    def populate_form(self):
        ...
        self.summary_text = tk.Text(self.scrollable_frame, width=95, height=5)
        self.summary_text.insert("1.0", self.data.get("summary", ""))
        self.summary_text.pack(anchor="w")

        # Core Expertise
        ttk.Label(self.scrollable_frame, text="CORE EXPERTISE", font=("Helvetica", 11, "bold")).pack(anchor="w", pady=(15, 5))
        exp_dict = self.data.get("expertise", {})
        self.exp_dict_entries = {}
        for cat, val in exp_dict.items():
            f = ttk.Frame(self.scrollable_frame)
            f.pack(fill=tk.X, pady=2)
            ttk.Label(f, text=f"{cat}:", width=30, font=("Helvetica", 8, "bold")).pack(side=tk.LEFT)
            ent = ttk.Entry(f, width=65)
            ent.insert(0, val)
            ent.pack(side=tk.LEFT, fill=tk.X, expand=True)
            self.exp_dict_entries[cat] = ent
        ...
        for job in self.data.get("experience", []):
            ...
            c_e = ttk.Entry(r1, width=25); c_e.insert(0, job.get("company", "")); c_e.pack(side=tk.LEFT, padx=(5,10))
            ttk.Label(r1, text="Role:").pack(side=tk.LEFT)
            r_e = ttk.Entry(r1, width=35); r_e.insert(0, job.get("role", "")); r_e.pack(side=tk.LEFT, padx=5)

            r2 = ttk.Frame(jf); r2.pack(fill=tk.X, pady=2)
            ttk.Label(r2, text="Meta/Location:").pack(side=tk.LEFT)
            m_e = ttk.Entry(r2, width=70); m_e.insert(0, job.get("meta", "")); m_e.pack(side=tk.LEFT, padx=5)

            ttk.Label(jf, text="Bullets (One per line):").pack(anchor="w", pady=(4, 2))
            b_t = tk.Text(jf, width=90, height=5)
        ...
        self.edu_text = tk.Text(self.scrollable_frame, width=95, height=3)
        ...
        self.achieve_text = tk.Text(self.scrollable_frame, width=95, height=3)
        ...
        self.lang_entry = ttk.Entry(self.scrollable_frame, width=95)
    ...
    def extract_form_data(self):
        return {
            ...
            "expertise": {cat: ent.get().strip() for cat, ent in self.exp_dict_entries.items()},
            "experience": [
                {
    ...
    def build_pdf_bytes(self):
        ...
        if data.get("expertise"):
            add_heading("Core Expertise")
            for k, v in data["expertise"].items():
    ...
    def update_page_count(self):
        try:
            ...
            self.page_label.config(text="Total Pages: Error")
